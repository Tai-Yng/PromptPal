...
- [ ] 3.1 QuickInject 顶部自动聚焦搜索框，即输即滤 title/content/tags（独立轻量过滤，不写主面板 store）——验证：手测即输即滤、清空恢复、关窗重开归零
- [ ] 3.2 列表改为收藏优先→useCount 降序，列表项加分类色点——验证：手测排序与色点展示
- [ ] 3.3 回车复制流程接入变量填空：有变量走填空后复制关窗、无变量直接复制关窗，useCount 递增，Esc 关窗不复制——验证：手测全部四条路径
- [ ] 3.4 阶段三 commit（`feat`）并推送——验证：本地 tsc+build+测试脚本全绿
